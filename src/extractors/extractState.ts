...
import { extractClimate, type ClimateState } from './extractClimate';
import { extractCharacters } from './extractCharacters';
import { extractScene, shouldExtractScene } from './extractScene';
import { setExtractionStep, type ExtractionStep } from './extractionProgress';
...
export interface ExtractionResult {
  state: TrackedState;
  raw: Record<string, string>;  // Raw responses from each extractor
}

export interface ExtractionOptions {
  forceSceneExtraction?: boolean;  // Force scene extraction even if not assistant turn
}
...
export async function extractState(
  ...
  try {
    ...
    const shouldRunScene = options.forceSceneExtraction ||
      shouldExtractScene(messageId, isAssistantMessage, isInitial);
